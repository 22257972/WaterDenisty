...
Beta_All <- array(data = NA, dim = c(n_para + n_basis, n_times, Burnin + Samples))
...
Eta_Squared_All <- rep(NA, Burnin + Samples)
...
OmegaGP <- matrix(0, nrow = n_depths, ncol = n_depths)
for (i in 1:n_depths) {
  for(j in 1:n_depths) {
    if(i <= j){
      X_small <- min(X[i,2],X[j,2])
      X_large <- max(X[i,2],X[j,2])
      OmegaGP[i,j] <- X_small^2 * (X_large - X_small/3)/2
      OmegaGP[j,i] <- OmegaGP[i,j]
    }
  }
}

D <- diag(eigen(OmegaGP)$values)
Q <- eigen(OmegaGP)$vectors
Z <- Q %*% D^(0.5)

X <- cbind(X, Z[,1:n_basis])

Y <- matrix(data = data.matrix(fulldata[1:n_times,-1]), nrow = n_depths, ncol = n_times, byrow = TRUE)

Beta <- matrix(data = NA, nrow = n_para + n_basis, ncol = n_times)

Sigma_Squared <- runif(1,1,100)
a <- 0.001
b <- 0.001

Mu <- c(rnorm(n = n_para, sd = 10), rep(0,n_basis))
Mu_Means <- rep(0, n_para + n_basis)
VCOV_Matrix_Mu <- diag(x = 10^4, nrow = n_para, ncol = n_para)

Tau_Squared <- runif(n_para, 1, 10^4)
Eta_Squared <- runif(1,1,100)
c <- 0.001
d <- 0.001
...
Standard_Normal_Simulation_For_Betas <- array(data = rnorm(n = n_times * (Burnin + Samples) * (n_para + n_basis)), dim = c(n_para + n_basis, n_times, Burnin + Samples))
...
Gamma_Simulation_For_Eta_Squared <- rgamma(n = Burnin + Samples, shape = (n_basis * n_times)/2 + c, rate = 1)
...
for (i in 1:(Burnin + Samples)) {
  ...
  # Beta's
  VCOV_Matrix_Inverse <- diag(1/c(Tau_Squared, rep(Eta_Squared, n_basis)))
  ...
  # Eta Squared's
  Eta_Squared <- (sum(Beta[(n_para+1):(n_para+n_basis),]^2)/2 + d)/Gamma_Simulation_For_Eta_Squared[i]
  
  # Store Data
  Beta_All[,,i] <- Beta
  Sigma_Squared_All[i] <- Sigma_Squared
  Mu_All[,i] <- Mu[1:n_para]
  Tau_Squared_All[,i] <- Tau_Squared
  Eta_Squared_All[i] <- Eta_Squared
...
plot(Eta_Squared_All[Sampled_Range], type = "l", col = "green", main = "eta_squared")
